...
def factor_analysis(data, important):
    model = decomposition.FactorAnalysis(n_components=2)
...
def PCA(data, important):
    ...
    X = pca.fit_transform(data_selected.iloc[:, :-1])
    pos = pandas.DataFrame()
    pos['X'] = X[:, 0]
    pos['Y'] = X[:, 1]
    target = data.columns[data.shape[1] - 1]
    ...
    axis = pos[pos[target] == 0].plot(kind='scatter', x='X', y='Y', color='blue', label=0)
    pos[pos[target] == 1].plot(kind='scatter', x='X', y='Y', color='red', label=1, ax=axis)
    print("explained_variance_ratio_ ->", pca.fit(data_selected.iloc[:, :-1].values).explained_variance_ratio_)
...
if __name__ == '__main__':
    ...
    factor_analysis(end_off, important)
    PCA(end_off, important)
